...
namespace AutoActivator.Services
{
    public class ExtractionService
    {
        ...
        public async Task<ExtractionResult> PerformExtractionAsync(string targetContract, string envSuffix, bool saveIndividualFile = true, bool isDemandId = false)
        {
            ...
            if (isDemandId)
            {
                var dtDemand = await db.GetDataAsync(SqlQueries.Queries["GET_CONTRACT_BY_DEMAND"], new Dictionary<string, object> { { "@DemandId", cleanedContract } });
            ...
            // On attend que les deux requêtes soient terminées
            await Task.WhenAll(lisaTask, eliaTask);

            var dtLisa = lisaTask.Result;
            var dtElia = eliaTask.Result;
            ...
            if (dtLisa.Rows.Count > 0)
            {
                ...
                await ExtractAndAppendTablesAsync(db, lisaTables, "@InternalId", internalId, sbLisa);
            }
            ...
            if (dtElia.Rows.Count > 0)
            {
                ...
                var dtDemand = await db.GetDataAsync(SqlQueries.Queries["GET_ELIA_DEMAND_IDS"], new Dictionary<string, object> { { "@EliaId", eliaUconId } });
                ...
                await ExtractAndAppendTablesAsync(db, eliaTables, "@EliaId", eliaUconId, sbElia);

                if (demandIds.Count > 0)
                {
                    var demandTables = new[] { "FJ1.TB5HDMD", "FJ1.TB5HDGM", "FJ1.TB5HDGD", "FJ1.TB5HPRO", "FJ1.TB5HEPT", "FJ1.TB5HDIC" };
                    await ExtractAndAppendTablesAsync(db, demandTables, "@DemandIds", string.Join(",", demandIds), sbElia);
                }
            }
